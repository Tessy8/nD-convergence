...
def make_corner_state(D, n_agents=4, corner='lower', jitter=0.0, rng=None):
    """
    Construct a symmetric 'corner' configuration:
      * all coordinates at lower_boundary (0.3) or upper_boundary (0.7),
      * optionally add small Gaussian jitter.

    Args:
        D        : internal dimension per agent (2, 4, 7, ...)
        n_agents: number of agents (default 4 as in your existing code)
        corner  : 'lower' -> all coords = lower_boundary
                  'upper' -> all coords = upper_boundary
        jitter  : std-dev of small noise around the corner (0.0 = exact corner)
        rng     : np.random.Generator for reproducible noise (optional)

    Returns:
        Y0 : 1D array of length n_agents * D
    """
    if corner == 'lower':
        val = lower_boundary
    elif corner == 'upper':
        ...
        raise ValueError("corner must be 'lower' or 'upper'")

    Y0 = np.full(n_agents * D, float(val))

    if jitter > 0.0:
        if rng is None:
            rng = np.random.default_rng()
        Y0 += jitter * rng.standard_normal(Y0.shape)
...
def analyze_corner_convergence(
        D_list=(2, 4, 7),
        corner='lower',
        T_total=40.0,
        dt=5e-3,
        ...
        jitter=0.0):
    """
    Probe 'weaker convergence in higher D' at a symmetric corner configuration.

    For each D in D_list:
      1) Build a symmetric corner state Y_corner (all coords at 0.3 or 0.7).
      2) Set the section phi_star = phi_bar(Y_corner), so Y_corner lies on the section.
      3) Compute ONE Poincaré return Jacobian (B-derivative) at that point.
      4) Print eigenvalues, transverse contraction c_perp, and log|λ| stats.

    Args:
        D_list : iterable of internal dimensions (e.g. (2,4,7))
        corner : 'lower' or 'upper' (0.3 or 0.7)
        T_total: max integration time to find one return
        dt     : macro step for the coarse OdePC pass (also used as dt_max)
        eps_J  : finite-diff step for Jacobian_rhs
        jitter : small noise around the exact corner (0.0 = exact corner)
    """
    for D in D_list:
        n_agents = 4  # same as your existing setup

        # 1) build base point at the corner
        Y0 = make_corner_state(D, n_agents=n_agents, corner=corner, jitter=jitter)

        # system parameters (same ones you used before)
        pars = dict(
            n_agents=n_agents,
            dim=D,
            speed=0.5,
        ...
        # 2) define section so that the corner is ON the section
        phi_star = phi_bar_of_state(Y0, pars)

        # 3) compute Poincaré return B-derivative at this corner base point
        Js, Ts = jacobians_poincare_full_D(
            ...
            eps_J=eps_J,
            phi_star=phi_star,
            max_returns=1,  # just one return at the corner
        )

        print(f"\n=== Corner experiment: D={D}, corner='{corner}', phi*={phi_star:.3f} ===")

        if not Js:
            print("  No returns found within T_total; try increasing T_total.")
            continue
        ...
        # identify neutral / degenerate modes as before
        tol_neutral = 1e-3
        neutral_mask = (np.abs(mags - 1.0) < tol_neutral) | (mags < 1e-8)
        transverse_mask = ~neutral_mask

        transverse_eigs = eigvals[transverse_mask]
        transverse_mags = np.abs(transverse_eigs)

        if transverse_mags.size > 0:
            c_perp = transverse_mags.max()
            log_abs = np.log(transverse_mags)
            avg_log_abs = log_abs.mean()
            min_log_abs = log_abs.min()
            max_log_abs = log_abs.max()
            # approximate max transverse Lyapunov exponent per unit time
            lam_max_per_time = max_log_abs / Delta
        else:
            c_perp = 1.0
            avg_log_abs = min_log_abs = max_log_abs = 0.0
            lam_max_per_time = 0.0

        print(f"  Return [{t_s:.3f} → {t_e:.3f}] Δt = {Delta:.3f}")
        print(f"  Eigenvalues ({len(eigvals)} total):")
        print(f"    {eigvals}")

        print(f"  Transverse modes: {transverse_eigs.size}")
        print(f"    c_perp (max |λ|)         = {c_perp:.6g}")
        print(f"    mean(log|λ|) (transverse)= {avg_log_abs:.4f}")
        print(f"    min(log|λ|) (transverse) = {min_log_abs:.4f}")
        print(f"    max(log|λ|) (transverse) = {max_log_abs:.4f}")
        print(f"    λ_max_per_time ≈ {lam_max_per_time:.4f} (per unit time)")
...
# ------------------------- (existing viz/animation code) --------------------
# ... keep your QQ plots, projections, and animation helpers unchanged ...
# (omitted here for brevity; they don't affect the Poincaré Jacobian)

def analyze_corner_growth_distribution(
        D_list=(2, 4, 7),
        corner='lower',
        T_total=40.0,
        dt=5e-3,
    ...
    """
    For each D:
      * Build a symmetric corner state (all coords at 0.3 or 0.7).
      * Define the Poincaré section so that the corner lies on it.
      * Compute ONE Poincaré return B-derivative J_ret at the corner.
      * Sample random unit vectors v and compute growth factors:
            g = ||J_ret v||,
        then log_g = log(g).
      * Plot histogram of log_g for each D, and print summary stats.

    Args:
        D_list   : iterable of internal dims, e.g. (2,4,7)
        corner   : 'lower' (0.3) or 'upper' (0.7)
        T_total  : max integration time to find one return
        dt       : macro step for coarse pass (also dt_max for J integration)
        eps_J    : finite-diff step for Jacobian_rhs
        jitter   : small noise around exact corner (0.0 = exact corner)
        n_samples: number of random unit vectors v per D
    ...
    for D in D_list:
        n_agents = 4  # consistent with your earlier setup

        # 1) base point at the corner
        Y0 = make_corner_state(D, n_agents=n_agents, corner=corner, jitter=jitter)
        ...
        # 2) section so that the corner lies on it
        phi_star = phi_bar_of_state(Y0, pars)

        # 3) single Poincaré B-derivative J_ret at the corner
        Js, Ts = jacobians_poincare_full_D(
            Y0, pars,
            T_total=T_total,
            dt=dt,
        ...
        print(f"\n=== Growth distribution at corner: D={D}, corner='{corner}', phi*={phi_star:.3f} ===")

        if not Js:
            print("  No returns found within T_total; skipping this D.")
            continue
        ...
        # 4) sample random unit vectors v
        V = rng.standard_normal((n_samples, N))
        V /= np.linalg.norm(V, axis=1, keepdims=True)  # normalize each row

        # 5) apply J_ret: w = J_ret v
        W = V @ J_ret.T
        norms = np.linalg.norm(W, axis=1)  # growth factors ||J_ret v|| (since ||v||=1)
        log_g = np.log(norms)

        # summary stats
        mean_log = float(np.mean(log_g))
        std_log  = float(np.std(log_g))
        min_log  = float(np.min(log_g))
        max_log  = float(np.max(log_g))
        # approximate max Lyapunov exponent per time, from worst sampled direction
        lam_max_per_time = max_log / Delta

        print(f"  Δt (return time) = {Delta:.3f}")
        print(f"  samples          = {n_samples}")
        print(f"  mean(log||Jv||)  = {mean_log:.4f}")
        print(f"  std(log||Jv||)   = {std_log:.4f}")
        print(f"  min(log||Jv||)   = {min_log:.4f}")
        print(f"  max(log||Jv||)   = {max_log:.4f}")
        print(f"  λ_max_per_time ≈ {lam_max_per_time:.4f} (per unit time; from sampled max)")

        all_stats.append((D, mean_log, std_log, min_log, max_log, lam_max_per_time))

        # 6) histogram for this D
        ax.hist(
            log_g,
            bins=bins,
            density=True,
    ...
    ax.set_xlabel("log(||J_ret v||) over one return")
    ax.set_ylabel("density")
    ax.set_title(f"Growth distribution at {corner} corner (phi* corner-aligned)")
    ax.legend()
    plt.tight_layout()

    print("\n=== Summary over D ===")
    for (D, mean_log, std_log, min_log, max_log, lam_max_per_time) in all_stats:
        print(f"D={D}: mean={mean_log:.4f}, std={std_log:.4f}, "
              f"min={min_log:.4f}, max={max_log:.4f}, "
              f"λ_max/time≈{lam_max_per_time:.4f}")
...
if __name__ == "__main__":
    ...
    #     D_list=(7, 4, 2),
    #     T_total=500.0,   # enough time to collect several returns
    #     dt=5e-3,
    #     eps_J=1e-6,
    ...
    # analyze_corner_convergence(
    #     D_list=(7, 4, 2),
    #     corner='lower',    # try 'upper' for 0.7
    #     T_total=40.0,
    #     dt=5e-3,
    #     eps_J=1e-6,
    #     jitter=0.0         # set e.g. 1e-4 to probe a small neighborhood of the corner
    # )

    analyze_corner_growth_distribution(
        D_list=(2, 4, 7),
        corner='lower',   # or 'upper'
        T_total=40.0,
        dt=5e-3,
        eps_J=1e-6,
        jitter=0.0,       # or small value e.g. 1e-4 to probe a neighborhood
        n_samples=5000,
        rng_seed=12345,
        bins=60
    )
